 ...
 *
 */
public class AddKzDdActivity extends Activity implements OnClickListener{
	private static final String tag = "AddKzDdActivity";
	private EditText mbxUsername; // 帐号编辑框
	private EditText innerId;
	private Button mAddDdBtn;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_add_kz_dd);
		Intent intent = getIntent();
		mbxUsername = (EditText) findViewById(R.id.add_kz_dd_user_edit);
		innerId = (EditText) findViewById(R.id.add_kz_dd_inner_id);
		
		mAddDdBtn = (Button) findViewById(R.id.add_kz_dd_btn);
		mAddDdBtn.setOnClickListener(this);
		
		mBackBtn = (Button) findViewById(R.id.add_kz_dd_reback_btn);
	...
	public void onClick(View v) {
		...
		switch(v.getId()) {
		case R.id.add_kz_dd_btn:
			add_Dd();
			break;
		case R.id.add_kz_dd_reback_btn:
			finish();
			break;
		default:
			break;
	...
	public void add_Dd() 
	{
		...
		else 
		{
			...
			if (ThreeFragmentKzDd.getDataByTitle(bxUsername) != null)
			{
			...
			map.put("btn", "setting");
			int result = ThreeFragmentKzDd.addData(map);
			switch (result){
			case 0: 
