...
public class ProfileActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		System.out.println("----------------------"+vcardEntity);

		if (vcardEntity != null) {
			String nickName = vcardEntity.getNickname();
			String company = vcardEntity.getCompany();
			String job = vcardEntity.getJob();
			String signature = vcardEntity.getSignature();
			if ("".equals(nickName)) {
	...
	protected void onActivityResult(final int requestCode,
		...
		if (requestCode == 1) {
			...
			if (requestCode == KFImageUtils.REQUEST_CODE_GETIMAGE_BYSDCARD) {
				...
				if (!"photo".equals(KFMediaUtils.getContentType(attFormat))) {
					Toast.makeText(ProfileActivity.this, "图片格式错误",
