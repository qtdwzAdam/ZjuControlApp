 ...
 *
 */
public class AddKzKaiGActivity extends Activity implements OnClickListener{
	private static final String tag = "AddKzKaiGActivity";
	private EditText mbxUsername; // 帐号编辑框
	private EditText innerId;
	private Button mAddKaiGBtn;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_add_kz_kaig);
		Intent intent = getIntent();
		mbxUsername = (EditText) findViewById(R.id.add_kz_kaig_user_edit);
		innerId = (EditText) findViewById(R.id.add_kz_kaig_inner_id);
		
		mAddKaiGBtn = (Button) findViewById(R.id.add_kz_kaig_btn);
		mAddKaiGBtn.setOnClickListener(this);
		
		mBackBtn = (Button) findViewById(R.id.add_kz_kaig_reback_btn);
	...
	public void onClick(View v) {
		...
		switch(v.getId()) {
		case R.id.add_kz_kaig_btn:
			add_KaiG();
			break;
		case R.id.add_kz_kaig_reback_btn:
			finish();
			break;
		default:
			break;
	...
	public void add_KaiG() 
	{
		...
		else 
		{
			...
			if (ThreeFragmentKzKaig.getDataByTitle(bxUsername) != null)
			{
			...
			map.put("btn", "setting");
			int result = ThreeFragmentKzKaig.addData(map);
			switch (result){
			case 0: 
